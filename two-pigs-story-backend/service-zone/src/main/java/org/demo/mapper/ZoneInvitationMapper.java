...
public interface ZoneInvitationMapper extends BaseMapper<ZoneInvitationPO> {
    ...
    @Select("SELECT * FROM zone_invitations WHERE receiver = #{userId}")
     ...
     * @return 邀请信息状态
     */
    @Select("SELECT status FROM zone_invitations WHERE id = #{id}")
     ...
     * @return 是否成功更新
     */
    @Update("UPDATE zone_invitations SET status = #{status}, handle_time = #{time} WHERE id = #{invitationId}")
